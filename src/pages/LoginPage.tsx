      ...
      <div className="max-w-md w-full space-y-8 bg-white dark:bg-[#1E1E1E] p-8 rounded-2xl shadow-xl border border-gray-100 dark:border-[#2E2E2E] mt-2 md:mt-4 transition-colors duration-200">
        <div>
          ...
          <div className="mt-4 flex items-center justify-center gap-2 text-[10px] md:text-xs uppercase tracking-[0.2em] text-gray-500 dark:text-[#AAAAAA]">
            <span>or</span>
            <Link to="/register" className="font-medium text-[#1a1a1a] dark:text-[#C1A98F] hover:text-gray-500 dark:hover:text-white transition-colors pb-0.5 border-b border-[#1a1a1a] dark:border-[#C1A98F] hover:border-gray-500">
        ...
        {isVerifiedSuccess && (
          <div className="bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800/60 text-emerald-800 dark:text-emerald-300 p-4 rounded-xl flex items-start space-x-3 text-sm">
        ...
        {isAlreadyVerified && !isVerifiedSuccess && (
          <div className="bg-blue-50 dark:bg-blue-950/40 border border-blue-200 dark:border-blue-800/60 text-blue-800 dark:text-blue-300 p-4 rounded-xl flex items-start space-x-3 text-sm">
        ...
        {/* URL Error Alert */}
        {urlError && !error && (
          <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 text-amber-900 dark:text-amber-300 p-4 rounded-xl flex items-start space-x-3 text-sm">
        ...
        {isRegisteredSuccess && !isVerifiedSuccess && !isAlreadyVerified && (
          <div className="bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800/60 text-amber-900 dark:text-amber-300 p-4 rounded-xl flex items-start space-x-3 text-sm">
            <Mail className="w-5 h-5 text-amber-700 dark:text-amber-400 shrink-0 mt-0.5" />
            <div>
        ...
        {/* Error Alert */}
        {error && (
          <div className="bg-red-50 dark:bg-red-950/40 border border-red-200 dark:border-red-800/60 text-red-700 dark:text-red-300 p-4 rounded-xl space-y-2 text-sm">
            ...
            {isUnverified && (
              <div className="pt-2 border-t border-red-200/60 dark:border-red-800/40 mt-2">
                {resendStatus === 'sent' ? (
                  <p className="text-emerald-700 dark:text-emerald-400 font-medium text-xs flex items-center gap-1.5">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <button
                      ...
                      className="text-xs text-[#8C6D4F] dark:text-[#C1A98F] hover:text-[#1a1a1a] dark:hover:text-white font-semibold underline flex items-center gap-1.5 transition-colors"
                    >
                      <RefreshCw className={`w-3.5 h-3.5 ${resendStatus === 'loading' ? 'animate-spin' : ''}`} />
                      {resendStatus === 'loading' ? 'Sending...' : 'Resend verification email'}
                    </button>
                    {resendStatus === 'error' && (
                      <span className="text-[11px] text-red-600 dark:text-red-400">{resendMessage}</span>
                    )}
                  </div>
                )}
              </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div className="rounded-md space-y-4">
            <div>
              <label htmlFor="email-address" className="block text-[10px] uppercase tracking-[0.3em] text-gray-400 dark:text-gray-500 font-medium mb-2">Email Address</label>
              <div className="flex items-center border-b border-gray-200 dark:border-[#2E2E2E] focus-within:border-[#1a1a1a] dark:focus-within:border-[#C1A98F] transition-colors duration-300 py-3">
                ...
                <input
                  ...
                  autoComplete="email"
                  required
                  className="w-full bg-transparent text-[15px] text-[#1a1a1a] dark:text-[#F5F5F5] placeholder-gray-300 dark:placeholder-gray-600 font-light tracking-wide focus:outline-none"
            ...
            <div>
              <label htmlFor="password" className="block text-[10px] uppercase tracking-[0.3em] text-gray-400 dark:text-gray-500 font-medium mb-2">Password</label>
              <div className="flex items-center border-b border-gray-200 dark:border-[#2E2E2E] focus-within:border-[#1a1a1a] dark:focus-within:border-[#C1A98F] transition-colors duration-300 py-3">
          ...
          <div>
            <button
              type="submit"
              disabled={loading}
              className="mt-6 w-full flex justify-center items-center gap-3 bg-[#1a1a1a] dark:bg-[#C1A98F] text-white dark:text-black text-[12px] uppercase tracking-[0.45em] py-5 px-14 hover:bg-gray-800 dark:hover:bg-[#d6beab] transition-colors duration-200 font-medium disabled:opacity-50 disabled:cursor-not-allowed cursor-pointer"
            >
        ...
        <div className="pt-2 text-center border-t border-gray-100 dark:border-[#2E2E2E]">
          <button
            ...
            className="text-[11px] uppercase tracking-[0.2em] text-[#8C6D4F] dark:text-[#C1A98F] hover:text-[#1a1a1a] dark:hover:text-white transition-colors"
          >
            {showResendModal ? 'Hide resend options' : "Didn't receive verification email? Resend"}
          </button>

          {showResendModal && (
            <div className="mt-4 p-4 bg-[#FAF7F5] dark:bg-[#252525] border border-[#EDE6DF] dark:border-[#2E2E2E] rounded-xl text-left space-y-3">
              <span className="block text-[11px] text-[#1a1a1a] dark:text-[#F5F5F5] font-medium tracking-wide">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 bg-white dark:bg-[#1E1E1E] border border-gray-200 dark:border-[#2E2E2E] px-3 py-2 text-xs text-[#1a1a1a] dark:text-[#F5F5F5] rounded outline-none focus:border-[#1a1a1a] dark:focus:border-[#C1A98F]"
                />
                <button
                  type="button"
                  onClick={() => handleResendVerification()}
                  disabled={resendStatus === 'loading'}
                  className="bg-[#1a1a1a] dark:bg-[#C1A98F] text-white dark:text-black text-[10px] uppercase tracking-widest px-3 py-2 rounded hover:bg-gray-800 dark:hover:bg-[#d6beab] transition-colors disabled:opacity-50"
                >
