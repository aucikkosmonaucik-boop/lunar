      ...
      if (existingOrder) {
        ...
        savedOrder = await (prisma as any).order.create({
          data: {
            ...
            carrier: session.metadata?.carrier || 'AN_POST',
            carrierName: session.metadata?.carrierName || 'An Post',
            estimatedDelivery: session.metadata?.estimatedDelivery || '1 – 3 Business Days',
            status: 'Paid',
            paymentStatus: 'paid',
